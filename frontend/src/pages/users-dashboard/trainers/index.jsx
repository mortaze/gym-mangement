...
export default function TrainingRequestsPage() {
  ...
  const fetchRequests = async () => {
    try {
      // 1️⃣ گرفتن currentUser از sessionStorage
      const rawUser = sessionStorage.getItem("currentUser");
      const currentUser = rawUser ? JSON.parse(rawUser) : null;

      if (!currentUser?._id) {
        console.warn("کاربر وارد نشده یا currentUser معتبر نیست");
        setRequests([]); // یا هر رفتار دیگه که میخوای
        return;
      }

      // 2️⃣ گرفتن همه درخواست‌ها
      const res = await fetch("http://localhost:7000/api/training-requests");
      const data = await res.json();

      if (data.success && Array.isArray(data.requests)) {
        // 3️⃣ فیلتر کردن فقط درخواست‌های کاربر فعلی
        const userRequests = data.requests.filter(
          (req) => req.userId?._id === currentUser._id,
        );

        setRequests(userRequests);
      } else {
        console.warn("داده‌ها معتبر نیستند یا success=false");
        setRequests([]);
      }
    } catch (err) {
      console.error("خطا در fetchRequests:", err);
      setRequests([]);
    } finally {
      setLoading(false);
    }
