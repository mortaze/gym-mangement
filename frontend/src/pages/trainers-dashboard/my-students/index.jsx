    ...
    <DashboardLayout>
      <div className="p-4 sm:p-8 min-h-screen bg-[#0f1115] rounded-[2.5rem] border border-gray-800 shadow-2xl" dir="rtl">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h2 className="text-3xl font-black text-white uppercase tracking-tight">ورزشکاران من</h2>
            <p className="text-gray-500 text-xs mt-1 uppercase tracking-widest">My Students</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[#1a1d23] border border-gray-800 rounded-2xl py-3 pr-12 pl-4 text-white text-sm outline-none focus:border-yellow-400 transition-all"
            />
            {search && (
...
function StudentCard({ student, onSelect, lastActivity }) {
  ...
  return (
    <div
      onClick={onSelect}
      className="bg-[#1a1d23] rounded-3xl border border-gray-800 hover:border-yellow-400/60 transition-all shadow-xl overflow-hidden cursor-pointer group"
    >
      <div className="p-5 border-b border-gray-800 flex items-center gap-3">
        <div className="w-12 h-12 flex items-center justify-center bg-gray-800 rounded-xl text-yellow-400 font-black text-lg border border-gray-700 shrink-0">
          {student.name?.charAt(0) || "?"}
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-white font-black leading-tight truncate">{student.name || "—"}</p>
          <p className="text-[11px] text-gray-500 mt-0.5">{student.employeeCode || "—"}</p>
        </div>
      </div>

      <div className="p-5 space-y-3 text-sm">
        <div className="flex items-center justify-between">
          <span className="text-gray-500 text-xs">وضعیت عضویت</span>
        ...
        <div className="flex items-center justify-between">
          <span className="text-gray-500 text-xs">آخرین حضور</span>
        ...
        {student.activeProgramTitle && (
          <div className="flex items-center justify-between">
            <span className="text-gray-500 text-xs">برنامه</span>
        ...
        {student.bmi && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-white font-black text-xs">{student.bmi}</span>
          </div>
        )}

        <div className="pt-2">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-gray-500 text-xs">پیشرفت</span>
...
function StudentDetailPanel({ student, onClose, lastActivity }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-lg bg-[#0f1115] border-r border-gray-800 h-full overflow-y-auto animate-slide-in">
        <div className="sticky top-0 bg-[#0f1115] border-b border-gray-800 p-4 flex items-center justify-between z-10">
          <h3 className="text-lg font-black text-white">جزئیات ورزشکار</h3>
        ...
        <div className="p-6 space-y-6">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 flex items-center justify-center bg-gray-800 rounded-2xl text-yellow-400 font-black text-2xl border border-gray-700">
              {student.name?.charAt(0) || "?"}
            </div>
            <div>
              <h2 className="text-2xl font-black text-white">{student.name}</h2>
              <p className="text-gray-500 text-sm">{student.employeeCode}</p>
              {student.email && <p className="text-gray-600 text-xs mt-0.5">{student.email}</p>}
            </div>
          </div>
          ...
          <div className="bg-[#1a1d23] rounded-2xl p-4 space-y-3">
            <h4 className="text-white font-black text-sm">فعالیت</h4>
            <div className="flex items-center justify-between">
              <span className="text-gray-500 text-xs">آخرین حضور</span>
              <span className="text-gray-300 text-xs font-bold">{lastActivity(student.lastAttendance)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-gray-500 text-xs">پایان عضویت</span>
          ...
          <div className="bg-[#1a1d23] rounded-2xl p-4 space-y-3">
            <h4 className="text-white font-black text-sm">پیشرفت تمرینی</h4>
            <div className="flex items-center justify-between mb-2">
              <span className="text-gray-500 text-xs">امتیاز کلی</span>
          ...
          {student.activeProgramTitle && (
            <div className="bg-[#1a1d23] rounded-2xl p-4 space-y-3">
              <h4 className="text-white font-black text-sm">برنامه فعال</h4>
          ...
          {student.goals?.length > 0 && (
            <div className="bg-[#1a1d23] rounded-2xl p-4 space-y-3">
              <h4 className="text-white font-black text-sm">اهداف</h4>
              <div className="flex flex-wrap gap-2">
                {student.goals.map((g, i) => (
                  <span key={i} className="bg-gray-900 text-gray-300 px-3 py-1 rounded-xl text-xs font-bold border border-gray-800">
                    {g}
                  </span>
                ))}
              </div>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="bg-[#1a1d23] rounded-3xl border border-gray-800 overflow-hidden animate-pulse">
          <div className="p-5 border-b border-gray-800 flex items-center gap-3">
    ...
    <div className="flex flex-col items-center justify-center py-24 text-gray-500">
      ...
      {hasFilters ? (
        ...
        <>
          ...
          <p className="text-sm mt-2 text-gray-600">پس از ثبت درخواست تمرین توسط کاربران و تایید آن، ورزشکاران شما در این بخش نمایش داده می‌شوند</p>
        </>
      )}
    </div>
  );
